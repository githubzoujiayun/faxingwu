...
public class PublicJobUI extends FinalActivity implements OnClickListener {

	private ProgressDialog dialog;
	private SessionManager sm;
	private String str_job = "美发学徒s";
	public static Oauth2AccessToken accessToken;
	private Spinner province_spinner;
	private Spinner city_spinner;
	private Integer provinceId, cityId;
	private static final String[] jobs = { "美发学徒", "美发助理", "发型师", "烫染技师",
			"技术总监", "美发店长" };
	...
	class UpLoadImage extends AsyncTask<String, Integer, Response> {
		protected void onPreExecute() {
			dialog = ProgressDialog.show(PublicJobUI.this, "",
		...
		private Map<String, Object> getListInqVal() {
			...
			map.put("money",
					((EditText) findViewById(R.id.et_jobmoney)).getText());
			map.put("city", strCity);
			map.put("number",
					((EditText) findViewById(R.id.et_jobnumber)).getText());
			map.put("address",
					((EditText) findViewById(R.id.et_jobaddress)).getText());
			map.put("telephone",
					((EditText) findViewById(R.id.et_jobphone)).getText());
			map.put("store_name",
					((EditText) findViewById(R.id.et_jobstore_name)).getText());

			if (((CheckBox) findViewById(R.id.cb_jobmiantan)).isChecked()) {
				map.put("interviews", "1");

			} else {
				map.put("interviews", "0");
		...
		protected Response doInBackground(String... params) {
			...
			return conn.executeAndParse(Constant.URN_ADDJOB, getListInqVal());
		...
		protected void onPostExecute(Response res) {
			...
			if (res.isSuccessful()) {
				TispToastFactory.getToast(PublicJobUI.this, res.getMsg())
						.show();
				finish();
			} else {
